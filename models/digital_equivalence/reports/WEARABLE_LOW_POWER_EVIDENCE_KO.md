...
물리 보드와 ASIC silicon 전력은 측정하지 않았다. Vivado 및 GPDK045 power는 **ESTIMATED**이고, 전력과 latency의 곱은 명시한 조건에서만 **DERIVED**이다. Run-1/run-2/run-3/run-4/run-5 vectorless power와 run-2 conditioned activity windows로 ASIC 판정당 에너지를 산출하지 않았다. Matched 0.00000087 mW delta도 energy/decision이 아니며 AXI는 vectorless only다.
