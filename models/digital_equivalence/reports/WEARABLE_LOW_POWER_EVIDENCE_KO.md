...
| GPDK045 run-5 AXI closure / vectorless | setup +2.703 ns, hold 0 path, data transition 0, clock slew 0, internal DRC 0, total 3.58433691 mW | PARTIAL; generic signal-route block, PG·foundry sign-off 아님 |
| GPDK045 run-6 AXI guardband / vectorless | setup +2.602 ns, hold +0.010 ns, data transition 0, clock slew 0, internal DRC 0, total 3.71636663 mW | PARTIAL; 100 ps engineering uncertainty 뒤 residual, workload power·sign-off 아님 |
...
1. GPDK045 VDD/VSS power routing과 PG/IR 해소
...
물리 보드와 ASIC silicon 전력은 측정하지 않았다. Vivado 및 GPDK045 power는 **ESTIMATED**이고, 전력과 latency의 곱은 명시한 조건에서만 **DERIVED**이다. Run-1/run-2/run-3/run-4/run-5/run-6 vectorless power와 run-2 conditioned activity windows로 ASIC 판정당 에너지를 산출하지 않았다. Matched 0.00000087 mW delta도 energy/decision이 아니며 AXI는 vectorless only다.
