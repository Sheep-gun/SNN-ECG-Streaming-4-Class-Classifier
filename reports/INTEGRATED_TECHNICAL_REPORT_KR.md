...
2026년 8월 현재 데이터 분석, 알고리즘 개발, MATLAB, LTspice, XMODEL, Python, Exact C++, RTL 구현과 AXI IP 패키징, MicroBlaze 통합, End-to-End 및 FPGA 검증을 완료하였다. 이어 generic GPDK045에서 digital core-only Genus mapping, Conformal 논리 등가성, Innovus signal post-route와 IQuantus 추출까지 수행했다. 이 run-1 core baseline 후 run-2 scan-free core와 AXI-inclusive accelerator block을 매핑·배치·CTS·신호 배선·RC 추출하고 mapped-to-postroute LEC를 수행했다. Core wrapper canonical digital RTL 36/36과 actual raw XMODEL 4/4를 별도 cohort로 재현했으며 AXI 36-case replay를 뜻하지 않는다. 다만 slow-early 0.95·fast-late 1.05 engineering derate assumption에서 core/AXI hold·data-transition residual이 남았고, exploratory PG는 실패했으며 unmodified four-state gate run은 X를 남겨 physical closure, PG/sign-off, ASIC 제작은 후속 과제이다.
...
**표 6. GPDK045 digital block 구현과 PPA: run-1 core / run-2 core·AXI**
...
표 6은 run-1 historical baseline과 run-2 scan-free core·AXI block을 구분한다. Run-2는 scan-free functional mapping이고 DFT insertion·scan QoR 근거가 아니다. Clock slew와 internal DRC는 개선되었지만 core/AXI hold·data-transition residual이 남아 physical timing closure는 아니다. OCV는 slow-early 0.95와 fast-late 1.05를 둔 fixed engineering assumption이며 PG attempt는 실패했다. Forced gate 결과와 timing check를 끈 single-seed MAX-SDF pilot은 유한 seed의 조건부 초기화 민감도이지 unmodified GLS·power-up·SDF timing regression·signoff 근거가 아니다. Core activity window도 같은 forced seed 경계의 zero-delay estimate다. SAIF parse/annotation PASS는 numeric coverage PASS가 아니고, prefix와 matched delta는 Snapshot/decision 또는 energy/decision 근거가 아니다. Run-1은 [historical evidence](../verification/asic_gpdk45_core/README_KR.md)와 [historical PPA table](../tables/asic_gpdk45_ppa.csv), run-2는 [static evidence](../verification/asic_gpdk45_run2/README_KR.md), [PPA table](../tables/asic_gpdk45_run2_ppa.csv)와 [verification table](../tables/asic_gpdk45_run2_verification.csv)에 보존한다.
...
| ASIC exploratory implementation | scan-free core/AXI mapping, LEC, place/CTS/route, extracted timing | Run-2 scan-free core·AXI signal route·IQuantus·post-route LEC 완료 | 부분 달성. setup 양수·clock slew 0·internal DRC 0이지만 hold·data transition·PG/sign-off 미해소 |
...
본 설계 범위의 모듈 구현과 통합 및 검증을 완료하였으며, 표 9와 같이 분류 성능, 스트리밍 입력, FPGA 하드웨어 자원, timing 및 기능 정합 목표를 달성하였다. GPDK045 run-2는 scan-free core/AXI block의 setup, clock slew, internal DRC와 post-route LEC를 개선했지만 hold·data-transition, PG·fill·sign-off가 남아 부분 달성으로 평가했다. 장시간 처리와 저전력 목표도 각각 30분 입력 검증, 이상적 power-gating 산출, vectorless ASIC 추정과 제한된 core conditioned activity window에 근거하므로 조건부로 유지한다.

본 작품은 웨어러블 기기에 적용 가능한 저전력 반도체 IP를 지향한다. 관련 ECG 전용 ASIC을 조사한 결과, Abubakar 등의 65 nm TNN 기반 프로세서는 13종 비정상 리듬 검출에서 746 nW의 실측 전력을 달성했으며 [7], Zhang 등의 55 nm ANN 기반 프로세서는 개별 심박의 5-클래스 분류에서 12.88 µW를 보고하였다 [8]. 본 작품의 2.991 µW는 완전 power-gating 가정의 산출값이고 GPDK045 run-1 3.35554239 mW와 run-2 core/AXI 3.71626492/3.69335598 mW는 default activity의 vectorless 추정치다. Run-2 core activity 수치도 seed-conditioned zero-delay window 추정이므로, 서로 다른 workload·공정·계측 범위의 실리콘 실측 수치와 직접 우열을 비교하지 않는다. 또한 선행연구가 주로 개별 심박이나 짧은 이상 리듬을 검출·분류하는 데 비해, 본 작품은 연속 ECG에서 박동·리듬·파형 증거를 장시간 누적하여 기록 전체를 NSR, CHF, ARR, AF 중 하나로 판정한다는 차별점이 있다.

다만 이러한 저전력 가능성과 장시간 분류 구조의 실효성을 최종적으로 입증하려면, core/AXI hold·data-transition 해소, 성공한 power grid·IR·power gating과 physical fill, complete antenna data와 foundry DRC/LVS sign-off, unmodified four-state reset/power-up robustness, quantified annotation coverage를 갖춘 Snapshot/decision workload activity와 실리콘 전력 실측이 필요하다. 현재 실제 검증 입력은 30분이며, 24시간 정확도, 처리시간과 전력은 검증하지 않았다. 물리 AFE PCB, ADC silicon, pad/DFT/package, fabricated silicon과 임상 검증도 수행하지 않았다. 또한 공개 데이터베이스별 클래스 결합에 따른 database–class confounding이 남아 있으므로, 동일한 측정 환경에서 수집한 장시간 다중 클래스 ECG로 분류 성능과 실제 24시간 동작을 추가 검증해야 한다. 2.991 µW와 run-1 3.35554239 mW, run-2 core/AXI 3.71626492/3.69335598 mW 및 core conditioned activity windows는 조건부 산출 또는 추정이지 FPGA/ASIC 실측 소비전력이 아니다.
...
AFE–ADC는 LTspice 회로와 XMODEL로, SNN 분류 가속기는 Pure RTL과 AXI 기반 FPGA IP로 구현하였다. 아날로그부는 목표 ECG 대역과 60 Hz 간섭 억제 특성을 재현해 1 kSPS signed 12-bit 출력을 RTL에 전달하였다. 디지털부는 원시 파형을 저장하지 않고 사건·리듬·파형 증거를 60초와 30분의 두 시간 척도로 누적한다. 설계 고정 후 정확도 80.56%, Macro-F1 80.44%를 기록했으며, Pure RTL은 BRAM·DSP 0과 FPGA timing closure를 달성하고 Exact C++보다 49.36배 짧은 활성시간을 보였다. XMODEL–RTL은 36-case compact acceptance와 현재 보유 raw dump 4/36 재실행을 구분하고, MicroBlaze FPGA 36-case replay는 모두 XSim 기준값과 일치하였다. Generic GPDK045 run-1 historical core baseline에 이어 run-2에서는 scan-free core와 AXI-inclusive accelerator block을 mapping·CTS·signal route·RC extraction하고 mapped-to-postroute LEC를 수행했다. Run-2 core/AXI setup WNS는 +2.469/+2.781 ns이고 clock-tree slew 위반과 internal route DRC는 0이지만, hold WNS −0.008/−0.016 ns와 data-net max-transition 위반이 남아 physical timing closure는 아니다. Core wrapper canonical digital RTL 36/36과 실제 raw XMODEL 4/4는 별도 cohort이며 AXI 36-case replay가 아니다. Core/AXI LEC 6,178/6,287 compare point PASS는 논리 등가성 근거로만 사용한다. FPGA 연속 처리 142.0 mW, 이상적 2.991 µW, run-1 ASIC vectorless 3.35554239 mW와 run-2 core/AXI vectorless 3.71626492/3.69335598 mW는 서로 다른 가정의 추정치다. Run-2 core의 2.02536072/1.91083992/1.91084079 mW activity windows도 forced seed·zero-delay 경계의 추정이며 silicon power·energy/decision이 아니다.
...
MATLAB 설계부터 LTspice, XMODEL, 기준 모델, RTL/XSim, Vivado와 FPGA까지 단계별 검증하였다. 공개 디지털 ECG를 PWL 자극으로 재구성해 아날로그 모델 간 정합을 확인하고, 디지털부는 출력과 내부 상태를 기준 모델과 대조하였다. AFE–ADC와 Pure RTL의 compact acceptance는 36-case 정합을 기록하지만, 실제 raw XMODEL accepted dump의 현재 저장소 재실행은 4/36이며 나머지 32개 누락으로 전체 기준은 FAIL이다. AXI/IP와 MicroBlaze 통합 FPGA는 별도 36-case replay에서 정상 동작하였다. GPDK045에서는 run-1 historical core baseline을 보존하면서 run-2 scan-free core와 AXI-inclusive accelerator block까지 합성·배치·CTS·신호 배선·RC 추출·post-route LEC 범위를 확장했다. Run-2 core wrapper RTL은 canonical digital 36/36과 actual raw XMODEL 4/4에서 exact 일치했고, 별도의 post-route LEC도 core/AXI 모두 diff·abort·unknown 0이지만, AXI 36-case replay를 뜻하지는 않는다. Core/AXI hold·data-transition residual과 실패한 PG attempt도 남아 있다. Unmodified four-state gate run은 X였고 forced two-state seed 11/22/33 및 timing check를 끈 single-seed MAX-SDF pilot은 testbench-conditioned sampled sensitivity일 뿐 일반 GLS·reset/power-up·SDF timing regression·sign-off 근거가 아니다. Core activity power도 동일한 forced-seed zero-delay 경계의 window estimate이며 numeric annotation coverage, Snapshot/decision energy나 AXI activity를 입증하지 않는다. 따라서 기존 FPGA-only 근거보다 디지털 ASIC flow 근거는 보강됐지만 physical timing closure, 성공한 PG/IR, physical fill, foundry DRC/LVS, DFT·pad/package와 fabrication은 완료되지 않았다. 최종 분류 시험은 학습, 검증에 쓰인 record와 겹치지 않고 설계 과정에서 한 번도 사용하지 않은 fully held-out 데이터로, 가중치, 임계값을 고정한 뒤 단 한 번만 수행하였다.
