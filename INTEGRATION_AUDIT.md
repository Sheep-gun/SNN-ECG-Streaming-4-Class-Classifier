...
- Run-5 AXI는 50% floorplan에서 setup +2.703 ns, hold WNS/TNS/path 0, data max-transition 0, clock slew 0, internal DRC 0, LEC 6,287점 clean을 기록했다. Die area 증가를 허용한 area–closure tradeoff이며 foundry sign-off가 아니다.
- Run-6 AXI는 기존 100 ps hold uncertainty 뒤에 추가 hold slack 10 ps를 남기면서 setup +2.602 ns, data max-transition 0, clock slew 0, internal DRC 0, LEC 6,287점 clean을 유지했다. Guardband 비용과 foundry-signoff 경계를 함께 기록했다.
- 최종 run-6 Innovus checkpoint에서 full·placement-only·routing-only·clock-net·qrs_maf selection과 Metal1–Metal11 layer-group GIF를 `dumpToGIF`로 직접 출력하고 command status를 보존했다.
...
- 물리 AFE PCB, ADC silicon, 성공한 PG/IR, physical-only cell·metal fill, foundry DRC/LVS, DFT, pad/package/fabrication과 임상 검증은 수행하지 않았다.
