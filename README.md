...
Run-1 GPDK045 결과는 generic demonstration library의 historical core-only baseline이며 scan-capable cell·clock slew·hold·DRC 한계를 보존한다. Run-2는 scan-free core와 AXI-inclusive block으로 확장했지만 hold와 data-transition residual이 남았다. Run-3는 같은 constraint·OCV assumption에서 core hold·data-transition·clock-slew·internal-DRC를 모두 0 violation으로 닫았고 mapped-to-postroute LEC도 유지했다. AXI는 hold WNS/TNS/path를 0으로 만들었지만 data max-transition 264 nets/1,387 terminals와 clock slew 263 pins가 남아 full physical closure가 아니다. Slow-early 0.95와 fast-late 1.05는 foundry AOCV/POCV/LVF가 아닌 fixed engineering assumption이다. Exploratory PG는 실패했고 선택한 checkpoint는 signal-only이며 VDD/VSS가 unrouted이므로 PG/IR/EM 근거가 아니다. Run-3 vectorless power 3.72167787 mW(core)와 3.79286409 mW(AXI)는 workload 또는 실측값이 아니다. 물리 AFE PCB, ADC silicon, physical fill, foundry DRC/LVS, pad/package/fabrication, 실리콘 전력, 임상 검증과 실제 24시간 입력 검증은 수행하지 않았다.
